...
此脚本能够在 `Ubuntu 22.04 LTS` 系统上自动化运行两次不同温度下的 GROMACS 分子动力学模拟，按照指定步骤完成从坐标文件生成到 NPT 弛豫模拟的全过程，并执行后续的 RMSD、RMSF 和 DSSP 分析;  （ 这可能需要额外提供或修改对应的.mdp文件 ）
...
- `GROMACS_CMD`: 可选参数，自定义 GROMACS 命令，默认使用 `gmx`。
- `Force_DIR` : 力场文件，需要至于 sh 脚本所在目录
- 更多请阅读相关脚本
...
假设核心文件目录为 `a2`，MDP 文件目录为 `mdp`，脚本的执行会生成以下目录结构：
...
1. ENERGY分析 (默认输出体积变化图)
2. RMSD 分析
3. RMSF 分析
4. DSSP 分析(DSSP.dat文件将会置于{TMP_DIR}处)
